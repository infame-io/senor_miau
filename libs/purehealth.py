...
from settings import slack_client, PH_URL, PH_USERNAME, PH_PASSWORD, PH_CHANNEL
from libs import get_db_session, scheduler, phantomjs_cleaner
        ...
        if jobs.count() > 0:
            ...
            for job in jobs:
                ...
                for class_url in class_urls:
                    logging.info("Searching URL for class {} {} {}".format(job.name, job.date, job.time))
...
def book_classes():
    ...
    try:
        ...
        if jobs.count() > 0:
            ...
            slack_client.api_call("chat.postMessage", channel=PH_CHANNEL, text=jobs_msg, as_user=True)
            ...
            if confirmation_msg:
                slack_client.api_call("chat.postMessage", channel=PH_CHANNEL, text=confirmation_msg, as_user=True)
        session.close()
        scheduler.add_job(phantomjs_cleaner)
        return result
    except Exception as e:
        logging.error("Exception raised {}".format(e))
        session.close()
